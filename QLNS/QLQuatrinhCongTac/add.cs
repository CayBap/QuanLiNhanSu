            ...
            if (txtMaNV.Text !="")
            {
                ...
                if (txtNoiCongTac.Text !="")
                {
                    ...
                    if (txtChucVu.Text != "")
                    {
                        ...
                        {
                            lblthongbao.Text = "";
                            if(QTCT.addBUL(txtMaNV.Text, dtpTuNgay.Value, dtpDenNgay.Value, txtNoiCongTac.Text, txtChucVu.Text))
                            {

                                MessageBox.Show("thêm thành công", "Kết Quả", MessageBoxButtons.OK, MessageBoxIcon.Information);
                            }else
                     
                                MessageBox.Show("Mã nhân viên không có trong hồ sơ hoặc mã vào từ ngày trùng với nhân viên đã có", "Thông Báo",MessageBoxButtons.OK,MessageBoxIcon.Hand);
